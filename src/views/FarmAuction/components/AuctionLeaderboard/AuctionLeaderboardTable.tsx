    ...
    <>
      ...
      <GridCell isTopPosition={isTopPosition}>
        <SubMenu component={<EllipsisIcon height="16px" width="16px" />}>
          {projectSite && <a href={projectSite}>{t('Project Site')}</a>}
          {lpAddress && <a href={`http://pancakeswap.info/pool/${lpAddress}`}>{t('LP Info')}</a>}
          {account && <a href={getBscScanLink(account, 'address')}>{t('Bidder Address')}</a>}
        </SubMenu>
      </GridCell>
    </>
  )
