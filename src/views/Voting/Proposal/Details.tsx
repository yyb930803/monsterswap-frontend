...
import { Box, Card, CardBody, CardHeader, Flex, Heading, Text } from 'uikit'
    ...
    <Card mb="16px">
      ...
      <CardBody>
        <Flex alignItems="center" mb="8px">
          ...
          <a href={`${IPFS_GATEWAY}/${proposal.id}`}>{proposal.id.slice(0, 8)}</a>
        </Flex>
        <Flex alignItems="center" mb="8px">
          <Text color="textSubtle">{t('Creator')}</Text>
          <a href={getBscScanLink(proposal.author, 'address')}>{truncateWalletAddress(proposal.author)}</a>
        </Flex>
        <Flex alignItems="center" mb="16px">
          <Text color="textSubtle">{t('Snapshot')}</Text>
          <a href={getBscScanLink(proposal.snapshot, 'block')}>{proposal.snapshot}</a>
        </Flex>
        <DetailBox p="16px">
